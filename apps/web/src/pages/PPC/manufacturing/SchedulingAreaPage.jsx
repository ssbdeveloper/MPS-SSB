...
const TOGGLE_BASE =
  'inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#00b4d8] motion-reduce:transition-none';
const DATE_INPUT =
  'rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-semibold tabular-nums text-slate-800 focus:border-[#0096c7] focus:outline-none focus:ring-2 focus:ring-[#00b4d8]';
const STEP_BTN =
  'flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#00b4d8] disabled:cursor-not-allowed disabled:opacity-40 motion-reduce:transition-none';
const ACTION_BTN =
  'inline-flex min-h-[44px] items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#00b4d8] disabled:cursor-not-allowed disabled:opacity-50 motion-reduce:transition-none';
...
export default function SchedulingAreaPage() {
  ...
  const initialArea =
    initialAreaCode && AREA_BY_CODE[initialAreaCode.toUpperCase()]
  ...
  useEffect(() => {
    indexRef.current = reservationIndex;
  }, [reservationIndex]);
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                onClick={() =>
                  setReservation({ mode: 'ORDER', bayCode: selectedBay, order: selectedOrder })
                }
              ...
              <button
                ...
                onClick={() =>
                  setReservation({ mode: 'NONJOB', bayCode: selectedBay, order: null })
                }
    ...
    <main className="flex min-h-0 flex-1 flex-col gap-3 px-4 py-4 md:px-6">
      <section className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2.5">
            ...
            <>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <button
                  ...
                  disabled={cursorDate <= bounds.min}
                ...
                <button
                  type="button"
                  onClick={() => stepCursor(1)}
                  disabled={cursorDate >= bounds.max}
                  aria-label="Next day"
                  className={STEP_BTN}
                >
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                <label className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    min={bounds.min}
                    max={maxEnd}
                    onChange={(event) => applyRange(event.target.value, range.end)}
                    className={DATE_INPUT}
                  />
                </label>
                <label className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    min={range.start}
                    max={maxEnd}
                    onChange={(event) => applyRange(range.start, event.target.value)}
                    className={DATE_INPUT}
                  />
                </label>
              </div>
          ...
          {viewMode === 'timeline' && (
            <div
              className="inline-flex rounded-lg border border-slate-200 bg-white p-0.5"
              role="group"
              aria-label="Date window presets"
            >
        ...
        <p className="text-xs tabular-nums text-slate-500">
          {viewMode === 'timeline'
            ? `Read-only · max ${MAX_RANGE_DAYS} days`
            : 'Map shows reservations active on the selected date (all dates)'}
        </p>
      </section>
