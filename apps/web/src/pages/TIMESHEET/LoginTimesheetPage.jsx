...
const UNPROD_QUICK_MODES = {
  ...
  break: { code: '1520', label: 'COFFEE BREAKS' },
...
const NFC_ERROR_MESSAGES = {
  NotAllowedError: [
    'Izin NFC ditolak',
    'Akses NFC ditolak. Silakan izinkan akses NFC di browser settings.',
  ],
  NotSupportedError: ['NFC tidak didukung', 'Device ini tidak mendukung NFC atau NFC tidak aktif.'],
  NotReadableError: [
    'NFC tidak dapat dibaca',
    'NFC sedang digunakan aplikasi lain atau tidak dapat diakses.',
  ],
      ...
      >
        ...
        <div className="p-4 grid grid-cols-4 gap-2 flex-shrink-0">
          ...
          >
            ...
            <span className="text-[10px] font-bold leading-tight">Break</span>
          </button>
        ...
        <div className="bg-slate-50 border-t border-slate-200 px-4 py-3 overflow-y-auto">
          <div className="flex items-start gap-2 mb-2">
            <svg
              className="w-4 h-4 text-[#0096c7] flex-shrink-0 mt-0.5"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <span className="text-[11px] font-bold text-slate-800">
              Cara kerja pilihan Mode Unprod:
            </span>
          </div>
          <div className="space-y-1.5 ml-6 text-[10px] text-slate-700 leading-relaxed">
            <p>
              <span className="font-bold text-[#0096c7]">Single</span> – Scan NFC / input ID seperti
              biasa untuk pindah ke halaman Unproductive.
            </p>
            <p>
              <span className="font-bold text-[#0096c7]">Briefing</span> – Setiap scan NFC / input
              ID akan otomatis membuat activity <span className="italic">Briefing</span>.
            </p>
            <p>
              <span className="font-bold text-amber-600">Break</span> – Setiap scan NFC / input ID
              akan otomatis membuat activity <span className="italic">Break</span>.
            </p>
            <p>
              <span className="font-bold text-emerald-700">HouseKeeping</span> – Setiap scan NFC /
              input ID akan otomatis membuat activity <span className="italic">HouseKeeping</span>.
            </p>
          </div>
          <div className="mt-2.5 ml-6 space-y-1 text-[10px] text-slate-500 leading-relaxed">
            <p>
              Mode yang dipilih akan tetap aktif sampai kamu keluar dari menu. Ketika mode sudah
              aktif, Anda hanya perlu scan ID tanpa harus memilih kembali activity unprod tersebut.
            </p>
...
const LoginTimesheetPage = () => {
  ...
  const [selectedAction, setSelectedAction] = useState(
    () => sessionStorage.getItem('scanAction') || 'start'
  );
  ...
  const getUserByNfc = async (nfcid) => {
    try {
      const res = await fetch(
        `${API_BASE}/usernfc/nfcid/${encodeURIComponent(normalizeNfcId(nfcid))}`
      );
      if (res.status === 409) {
  ...
  const processUser = async (user) => {
    ...
    if (quickMode && UNPROD_QUICK_MODES[quickMode]) {
      ...
      if (success) {
        setStatus(`${label} berhasil dibuat`);
        showSuccessToast(`${user.full_name || 'User'} - ${label} berhasil!`);
      } else {
  ...
  const startNfcListener = async (action) => {
    ...
    if (!('NDEFReader' in window)) {
      ...
      toast.error(
        'Browser Anda tidak mendukung Web NFC. Gunakan Chrome di Android atau gunakan mode Eksternal.'
      );
      return;
    }
    ...
    try {
      ...
      const [statusMsg, alertMsg] = NFC_ERROR_MESSAGES[err.name] ?? [
        'Gagal scan NFC',
        `Gagal melakukan scan NFC: ${err.message}`,
      ];
      setStatus(statusMsg);
  ...
  return (
    <div className="h-dvh w-screen bg-slate-50 flex flex-col overflow-hidden">
      ...
      <main className="relative flex-1 flex flex-col justify-center items-center px-4 py-2 overflow-hidden">
        ...
        {}
        <div
          className="w-full max-w-[360px] landscape:max-w-[680px]
        ...
        >
          ...
          <button
            ...
            className={
              unprodQuickMode ? btnUnprodActive : isActive('unprod') ? btnActive : btnPrimary
            }
          >
      ...
      {}
      {showUnprodPanel && (
        <UnprodPanel panelRef={unprodPanelRef} onSelect={handleUnprodPanelSelect} />
      )}
